...
describe("parseServerEnv", () => {
  ...
  test("throws when SPOTIFY_CLIENT_ID is missing", () => {
    delete process.env.SPOTIFY_CLIENT_ID;
    delete process.env.SPOTIFY_CLIENT_SECRET;
    delete process.env.SPOTIFY_REFRESH_TOKEN;
    delete process.env.SPOTIFY_REDIRECT_URI;

    expect(() => parseServerEnv()).toThrow("Missing or invalid environment variables");
  });

  test("throws when SPOTIFY_REFRESH_TOKEN is missing", () => {
    process.env.SPOTIFY_CLIENT_ID = "test-client-id";
    process.env.SPOTIFY_CLIENT_SECRET = "test-client-secret";
    delete process.env.SPOTIFY_REFRESH_TOKEN;

    expect(() => parseServerEnv()).toThrow("SPOTIFY_REFRESH_TOKEN");
  });

  test("throws when SPOTIFY_REDIRECT_URI is not a valid URL", () => {
    process.env.SPOTIFY_CLIENT_ID = "test-client-id";
    process.env.SPOTIFY_CLIENT_SECRET = "test-client-secret";
    process.env.SPOTIFY_REFRESH_TOKEN = "test-refresh-token";
...
describe("parseAuthEnv", () => {
  ...
  test("throws when SPOTIFY_CLIENT_ID is missing", () => {
    ...
    expect(() => parseAuthEnv()).toThrow("SPOTIFY_CLIENT_ID and SPOTIFY_CLIENT_SECRET");
  });

  test("throws when SPOTIFY_CLIENT_SECRET is missing", () => {
    process.env.SPOTIFY_CLIENT_ID = "test-client-id";
    delete process.env.SPOTIFY_CLIENT_SECRET;
