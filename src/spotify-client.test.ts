...
describe("response caching", () => {
	...
	test("mutating requests (POST) clear related cache entries", async () => {
		let apiCallCount = 0;
		mockFetch(() => {
			apiCallCount++;
			return apiResponse({
		...
		// Cache a player endpoint
		await spotifyRequest(mockEnv, "/me/player/state");
		expect(apiCallCount).toBe(1);

		// POST to a player endpoint should invalidate /me/player* entries
		mockFetch(() => {
			apiCallCount++;
			return apiResponse({
				snapshot_id: "snap",
		...
		await spotifyRequest(mockEnv, "/me/player/play", "POST");
	...
	test("mutating requests (DELETE) clear related cache entries", async () => {
		let apiCallCount = 0;
		mockFetch(() => {
			apiCallCount++;
			return apiResponse({
		...
		// Cache a playlist endpoint
		await spotifyRequest(mockEnv, "/playlists/abc/tracks");
		expect(apiCallCount).toBe(1);

		// DELETE on playlist should invalidate /playlists* entries
		mockFetch(() => {
			apiCallCount++;
			return apiResponse({
				snapshot_id: "snap",
		...
		await spotifyRequest(mockEnv, "/playlists/abc/tracks", "DELETE", {
			uris: ["a"],
		});
...
describe("empty body handling", () => {
	...
	test("200 with non-JSON body on GET throws", async () => {
		mockFetch(
			() =>
		...
		await expect(
			spotifyRequest(mockEnv, "/me/player/currently-playing"),
		).rejects.toThrow("Invalid JSON response from Spotify");
	});

	test("200 with non-JSON body on PUT returns null", async () => {
		mockFetch(
			() =>
				new Response("WKnfnRC4wAbGxM3srAyTvLbDodE", {
					status: 200,
		...
		const result = await spotifyRequest(mockEnv, "/me/player/pause", "PUT");
		expect(result).toBeNull();
	});
